...
export const Route = createFileRoute("/_authenticated/admin/")({
  head: () => ({
    meta: [
      { title: "Admin panel | Sujala" },
      { name: "description", content: "Manage Sujala orders, complaints, products and payment settings." },
      { property: "og:title", content: "Admin panel | Sujala" },
      { property: "og:description", content: "Manage Sujala orders, complaints, products and payment settings." },
    ],
  }),
  component: AdminPage,
});
...
function AdminPage() {
  ...
  if (isAdmin === null) {
    return (
      <SiteShell>
        <div className="mx-auto max-w-7xl px-4 py-16">Checking permissions…</div>
      </SiteShell>
    );
  }
...
function AdminInner() {
  ...
  const products = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("products")
        .select("id, name, brand, price, discount_price, stock, is_active")
  ...
  const revenue = (orders.data ?? []).reduce((sum, order) => sum + Number(order.subtotal || 0), 0);
          ...
          <div className="flex gap-2">
            <Button variant="outline" asChild>
              <Link to="/admin/products">
                <Package className="mr-1.5 h-4 w-4" /> Add / edit products
              </Link>
            </Button>
            <Button variant="outline" asChild>
                  ...
                  return (
                    <tr key={order.id} className="align-top hover:bg-secondary/30">
                      <td className="px-4 py-3">
                        <div className="font-semibold text-primary">{order.order_number}</div>
                      ...
                      <td className="max-w-xs px-4 py-3 text-xs text-muted-foreground">
                        {order.address}
                        {(order.city || order.pincode) && <div>{[order.city, order.pincode].filter(Boolean).join(" — ")}</div>}
                        {order.notes && <div className="mt-1 italic">Note: {order.notes}</div>}
                      </td>
                      <td className="px-4 py-3 text-right font-semibold">{formatINR(Number(order.subtotal))}</td>
                      <td className="px-4 py-3">
                        <div className="text-xs font-medium">{order.payment_method === "cod" ? "COD" : "Bank"}</div>
                        <select
                          value={order.payment_status}
                          onChange={(e) => updateOrder.mutate({ id: order.id, payment_status: e.target.value })}
                          className="mt-1 rounded-md border border-input bg-background px-2 py-1 text-xs"
                        >
                          {PAY_STATUSES.map((status) => <option key={status} value={status}>{status.replace(/_/g, " ")}</option>)}
                        </select>
                      </td>
                      <td className="px-4 py-3">
                        <select
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {complaints.data?.map((complaint) => (
                  <tr key={complaint.id} className="align-top hover:bg-secondary/30">
                    <td className="px-4 py-3 font-semibold text-primary">{complaint.ticket_number}</td>
                    <td className="px-4 py-3">
                      <div className="font-medium">{complaint.name}</div>
                      <div className="text-xs text-muted-foreground">{complaint.phone}</div>
                      {complaint.email && <div className="text-xs text-muted-foreground">{complaint.email}</div>}
                    </td>
                    <td className="max-w-xs px-4 py-3 text-xs text-muted-foreground">
                      {complaint.address}
                      {complaint.city && <div>{complaint.city}</div>}
                    </td>
                    <td className="px-4 py-3">{complaint.category}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-semibold capitalize ${
                        complaint.priority === "emergency" ? "bg-destructive/15 text-destructive"
                        : complaint.priority === "high" ? "bg-warning/20 text-warning-foreground"
                    ...
                    <td className="px-4 py-3">
                      <select
                        ...
                        onChange={(e) => updateComplaint.mutate({ id: complaint.id, status: e.target.value })}
                        className="rounded-md border border-input bg-background px-2 py-1 text-xs"
                      >
                        {COMPLAINT_STATUSES.map((status) => <option key={status} value={status}>{status.replace(/_/g, " ")}</option>)}
                      </select>
                    </td>
                    <td className="px-4 py-3 text-xs text-muted-foreground">{new Date(complaint.created_at).toLocaleDateString()}</td>
                  </tr>
                ))}
                ...
                {products.data?.map((product) => (
                  <tr key={product.id} className="hover:bg-secondary/30">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="font-semibold text-primary">{formatINR(Number(product.discount_price ?? product.price))}</div>
                    ...
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${product.is_active ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}>
                        {product.is_active ? "Active" : "Hidden"}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <Button asChild size="sm" variant="outline">
                        <Link to="/admin/products" search={{ edit: product.id }}>
...
function Stat({ i: Icon, n, l, c, isString }: { i: typeof ShoppingBag; n: number | string; l: string; c: string; isString?: boolean }) {
  return (
