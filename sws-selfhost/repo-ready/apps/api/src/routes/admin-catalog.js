...
async function normalProduct(body, files) {
  ...
  // Upload each new file to Cloudinary and keep the returned https URL.
  const newImages = [];
  for (const file of files || []) {
    newImages.push(await uploadBuffer(file.buffer, 'sws/products'));
  }
  return {
    ...
    images: [
      ...(Array.isArray(oldImages) ? oldImages.filter(url => typeof url === 'string' && url.length > 0) : []),
      ...newImages
    ]
  };
}
