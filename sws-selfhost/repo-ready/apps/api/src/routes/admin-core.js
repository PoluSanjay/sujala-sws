...
router.put('/settings', asyncRoute(async (req, res) => {
  ...
  const settings = await AppSettings.findByIdAndUpdate(current._id, { $set: { free_shipping_threshold: input.free_shipping_threshold, flat_shipping_fee: input.flat_shipping_fee, gst_rate: input.gst_rate, hero_image_url: input.hero_image_url ?? current.hero_image_url ?? '', smtp } }, { new: true, runValidators: true });
  const plain = settings.toObject();
  if (plain.smtp?.pass) plain.smtp.pass = '••••••••';
  res.json({ settings: plain });
}));
